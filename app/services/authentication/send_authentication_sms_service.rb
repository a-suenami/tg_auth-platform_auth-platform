...
module Authentication
  class SendAuthenticationSmsService < BaseService
    extend T::Sig

    # SMS送信対象外の国コードリスト
    EXCLUDED_COUNTRY_CODE = T.let(%w[AF AZ BI BZ ET IQ LB LK LY MG PK PS RU SY TD TJ TN UZ ZM].freeze, T::Array[String])

    sig { params(phone_number: String, user_id: String, ip_address: String, delivery_type: T.nilable(String)).returns(User) }
    def execute!(phone_number:, user_id:, ip_address:, delivery_type:)
      user = User.active.find user_id

      # SMS検証がマニュアル対応のユーザは送信せずに終了
      return user if user.suppress_sms_verification

      if excluded_country_code?(phone_number)
        # SMS送信対象外の国コードチェック
        raise Exceptions::Authentication::NoSmsSupportedCountry
      end

      sms_rate_limit(phone_number:, user:, _ip_address: ip_address)

      ActiveRecord::Base.transaction do
        sms_verifier = Users::SmsVerifier.new(user:, phone_number:, verifier_type: :two_factor_authentication, ip_address:)
