...
module Authentication
  class SendVerificationSmsService < BaseService
    ...
    def sms_rate_limit(phone_number:, user:, _ip_address:)
      ...
      # 同一電話番号 5件/3hours
      raise Exceptions::Authentication::SmsSendLimit if Users::SmsVerifier.rate_limit_targets.where('created_at > ?', 3.hours.ago).where(phone_number:).count >= 5
      # 同一電話番号 10件/24hours
      raise Exceptions::Authentication::SmsSendLimit if Users::SmsVerifier.rate_limit_targets.where('created_at > ?', 24.hours.ago).where(phone_number:).count >= 10
      # 同一IP      100件/1hours
      # raise Exceptions::Authentication::SmsSendLimit if Users::SmsVerifier.rate_limit_targetswhere('created_at > ?', 1.hour.ago).where(ip_address:).count >= 100
      # 同一ユーザ   10件/24hours
      raise Exceptions::Authentication::SmsSendLimit if Users::SmsVerifier.rate_limit_targets.where('created_at > ?', 24.hours.ago).where(user:).count >= 10
    end
