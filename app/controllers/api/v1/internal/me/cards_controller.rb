...
module API::V1::Internal
  class Me::CardsController < ApplicationController
    sig { void }
    def show
      card_payment_gateway = Tenant.current!.card_payment_gateway
    ...
    def create_setup_intent
      ...
      if payment_customer_id.blank?
        ...
        T.must(current_user.payment_customer_id)
      end
      ...
      raise result.err_inner if result.is_a?(Mangrove::Result::Err)

      setup_intent = result.ok_inner

      render :create_setup_intent, locals: { setup_intent: }
    end
    ...
    def update
      ...
      when Tenant::CardPaymentGatewayEnum::Stripe
        ...
        when StripeRecord::SetupIntent::CreateCardPaymentMethodResult::AlreadyCreated
          raise Exceptions::Payments::Stripe::SetupIntent::AlreadyCreated
        when StripeRecord::SetupIntent::CreateCardPaymentMethodResult::InvalidStatus
          raise Exceptions::Payments::Stripe::SetupIntent::NotCompleted
        when StripeRecord::SetupIntent::CreateCardPaymentMethodResult::InvalidPaymentMethodType
          raise Exceptions::Payments::Stripe::SetupIntent::NotCard
    ...
    def destroy
      ...
      when Tenant::CardPaymentGatewayEnum::Stripe
        ...
        T.assert_type!(detached_result.ok_inner, TrueClass)
      else
        T.absurd(card_payment_gateway)
      end
